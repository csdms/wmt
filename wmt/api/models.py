...
from ..models import (models, users, components, tags)
...
class Models(object):
    ...
    def POST(self):
        ...
        id = models.new_model(payload['name'], payload['json'],
                              owner=get_username())
        return id
...
class Tags(object):
    @as_json
    def GET(self, id):
        return tags.get_model_tags(int(id))
    ...
    def POST(self, id):
        ...
        try:
            tags.tag_model(int(id), payload['tag'])
    ...
    def REMOVE(self, id, tag):
        try:
            tags.untag_model(int(id), int(tag))
        except tags.BadIdError:
            raise web.BadRequest('tag does not exist')

    @as_json
    def OPTIONS(self):
        web.header('Allow', 'GET, POST, REMOVE')
